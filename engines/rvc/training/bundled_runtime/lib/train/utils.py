...
def latest_checkpoint_path(dir_path, regex="G_*.pth"):
    ...
    f_list.sort(key=_sort_key)
    return f_list[-1]


def _figure_canvas_to_numpy(fig, np_module):
    fig.canvas.draw()

    if hasattr(fig.canvas, "buffer_rgba"):
        data = np_module.asarray(fig.canvas.buffer_rgba(), dtype=np_module.uint8)[..., :3].copy()
    elif hasattr(fig.canvas, "tostring_rgb"):
        data = np_module.frombuffer(fig.canvas.tostring_rgb(), dtype=np_module.uint8)
        data = data.reshape(fig.canvas.get_width_height()[::-1] + (3,))
    else:
        data = np_module.frombuffer(fig.canvas.tostring_argb(), dtype=np_module.uint8)
...
def plot_spectrogram_to_numpy(spectrogram, cmap=None):
    ...
    data = _figure_canvas_to_numpy(fig, np)
    plt.close()
    return data
