...
# TTS Audio Suite: REMOVED sox dependency (only needed for 25Hz models, we use 12Hz)
# Provide stub implementation to prevent import errors
class _SoxTransformerStub:
    def __init__(self):
        pass
    def norm(self, db_level=None):
        pass
    def build_array(self, input_array=None, sample_rate_in=None):
        return input_array if input_array is not None else None

class _SoxStub:
    Transformer = _SoxTransformerStub
...
class XVectorExtractor(nn.Module):
    def __init__(self, audio_codec_with_xvector):
        ...
        # TTS Audio Suite: Use sox transformer (or mock if not available)
    ...
    def sox_norm(self, audio):
        # TTS Audio Suite: Use sox normalization (or passthrough if mock)
        wav_norm = self.tfm.build_array(input_array=audio, sample_rate_in=16000)
        return wav_norm if wav_norm is not None else audio
