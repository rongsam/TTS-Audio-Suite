...
class KugelAudioModel(KugelAudioPreTrainedModel):
    def __init__(self, config):
        ...
        # Semantic tokenizer: encoder-only, used for voice cloning and generation
        # conditioning. Extracts semantic features from audio for combined embeddings.
        self.semantic_tokenizer = AutoModel.from_config(config.semantic_tokenizer_config).to(dtype)

        # Connectors map tokenizer outputs -> LM hidden space
    ...
    def strip_encoders(self):
        ...
        """
        pass  # Disabled to support voice cloning (we need the encoder)
